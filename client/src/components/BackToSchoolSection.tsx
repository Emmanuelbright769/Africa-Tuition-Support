...
export default function BackToSchoolSection() {
  ...
  const startCbt = useMutation({
    mutationFn: async (input: number | string | { childId: number | string; proctoringSessionId: string | number }) => {
      const childId = typeof input === "object" ? input.childId : input;
      if (typeof input !== "object") {
        setActiveChild(children.find((child) => String(child.id) === String(childId)) || null);
        setConsentOpen(true);
        throw new Error("Review the recording agreement before starting.");
      }
      const proctoringSessionId = input.proctoringSessionId;
      const cbt = normalize(await (await apiRequest("POST", `/api/back-to-school/children/${childId}/cbt/start`, { proctoringSessionId })).json());
  ...
  useEffect(() => {
    ...
    if (secondsLeft <= 0) { void finishCbt(); return; }
      ...
      apiRequest("POST", `/api/back-to-school/children/${activeChild.id}/cbt/report-cheat`, { eventType, description })
        .then(async (response) => {
          ...
          if (body?.terminalResult && !autoSubmitRef.current) {
            ...
            await proctoring.finalize("interrupted"); setResult(body.terminalResult); setSession(null); refresh();
          } else if (body?.shouldAutoSubmit && !autoSubmitRef.current) {
            autoSubmitRef.current = true;
            toast({ title: "Assessment auto-submitted", description: "Detected test-integrity activity has ended this attempt.", variant: "destructive" });
    ...
    return <section className="min-h-[620px] rounded-[2rem] bg-[#f6f1e8] p-4 text-[#203a4b] sm:p-8" data-testid="back-to-school-cbt">
      <div className="mx-auto max-w-3xl">
        ...
        <div className="mb-8 flex items-center justify-between">
          <button className="flex items-center gap-2 text-sm font-semibold text-[#5e7280] hover:text-[#203a4b]" onClick={() => void leaveCbt()}><ChevronLeft className="h-4 w-4" /> Leave assessment</button>
        ...
        <div className="mt-6 flex justify-end"><Button disabled={!selected || submitCbt.isPending || proctoring.status === "flushing"} onClick={() => questionIndex < session.questions.length - 1 ? setQuestionIndex(questionIndex + 1) : void finishCbt()} className="rounded-xl bg-[#234b5e] px-6 hover:bg-[#183b4b]">{questionIndex < session.questions.length - 1 ? "Next question" : "Finish assessment"} <ArrowRight className="ml-2 h-4 w-4" /></Button></div>
      </div>
    </section>;
  }

  if (consentOpen && activeChild) return <section className="space-y-6 rounded-[2rem] bg-[#f6f1e8] px-4 py-10 text-[#203a4b] sm:px-8" data-testid="back-to-school-proctoring-notice"><ProctoringNotice assessmentLabel="the spelling bee" busy={proctoring.status === "preparing"} error={proctoring.error} onStart={async () => { try { const id = await proctoring.start({ assessmentType: "kiddies", childId: activeChild.id }); startCbt.mutate({ childId: activeChild.id, proctoringSessionId: id! }); setConsentOpen(false); } catch { /* notice contains recovery */ } }} /></section>;
