...
function SponsorshipCohortSection() {
  ...
  return (
    <div className="space-y-6">
      <div className="mb-5">
        <h2 className="text-2xl font-bold flex items-center gap-2 mb-1">
          <HeartHandshake className="w-6 h-6 text-tsia-green" /> Sponsorship Cohort
        </h2>
        <p className="text-muted-foreground text-sm">Support African students — choose between a Scholarship code or a full Sponsorship cohort.</p>
      </div>

      <div className="flex gap-2 p-1 bg-muted rounded-xl w-fit">
        {(["scholarship", "sponsorship"] as const).map(t => (
          <button key={t} onClick={() => setTab(t)}
            className={`px-5 py-2 rounded-lg text-sm font-medium transition-all ${tab === t ? "bg-background shadow text-foreground" : "text-muted-foreground hover:text-foreground"}`}
            data-testid={`tab-cohort-${t}`}>
            {t === "scholarship" ? "🎓 Scholarship" : "🏛️ Sponsorship"}
          </button>
        ))}
      </div>
        ...
        <div className="space-y-4">
          <div className="rounded-2xl border bg-card p-5 space-y-3">
            <div className="flex items-start gap-3">
              ...
              <div>
                <h3 className="font-semibold text-base">Sponsorship Cohort</h3>
                <p className="text-sm text-muted-foreground mt-0.5">Sponsor a cohort of <strong>30 or more students</strong> at <strong>$3.30 per student</strong>. Pay via Naira bank transfer or card, receive a reusable master code, and share it with all sponsored students.</p>
              </div>
            </div>
...
export default function AffiliateDashboard() {
  ...
  const completeBotSession = async (isAutoOff: boolean, overrideActivatedAt?: number) => {
    ...
    if (!isAutoOff) setBotDeactivating(true);
  ...
  const deactivateBot = () => {
    botCompletingRef.current = false; // reset any stuck guard before manual deactivation
    completeBotSession(false);
  };
            ...
            <div className="flex items-center justify-between px-5 py-4 border-b border-white/10">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-white/50">Affiliate Account</p>
                </div>
              </div>
      ...
      <main className={activeSection !== "overview" ? "fixed inset-0 z-50 bg-background flex flex-col" : "container mx-auto px-4 pt-8 pb-20 max-w-5xl"}>
        ...
        <AnimatePresence mode="wait">
          <motion.div key={activeSection} variants={containerVariants} initial="hidden" animate="visible" className="space-y-6">
                        ...
                        <div className="bg-card p-4 flex items-center gap-3">
                          {cycleTimeComplete ? (
                            ...
                            <>
                              ...
                              <Button size="sm" variant="outline" onClick={deactivateBot} disabled={botDeactivating} data-testid="button-bot-deactivate" className="border-red-300 text-red-600 hover:bg-red-50 disabled:opacity-60">
                                {botDeactivating
                                  ? <><div className="w-3.5 h-3.5 border-2 border-red-400 border-t-transparent rounded-full animate-spin mr-1" /> Stopping...</>
                      ...
                      {!cycleTimeComplete && lockedPrincipal > 0 && (
                        <div className="space-y-2">
                          <div className="grid grid-cols-2 gap-2">
                            ...
                            <div className="bg-white/70 dark:bg-blue-900/30 rounded-lg px-3 py-1.5 flex items-center gap-2">
                              <div className={`w-1.5 h-1.5 rounded-full shrink-0 ${withdrawableAmt > 0 ? "bg-emerald-500" : "bg-gray-300 dark:bg-gray-600"}`} />
                              <div>
                                <p className="text-[10px] text-muted-foreground">Available to Withdraw</p>
                                <p className={`text-xs font-bold ${withdrawableAmt > 0 ? "text-emerald-600 dark:text-emerald-400" : "text-muted-foreground"}`}>
            ...
            {/* ── SPONSORSHIP COHORT ── */}
            {activeSection === "sponsorship_cohort" && (
              <motion.div variants={itemVariants}>
                <SponsorshipCohortSection />
              </motion.div>
            )}
            ...
            <>
              ...
              <div className="space-y-4 py-2">
                ...
                <div>
                  ...
                  {parseFloat(fundTradeAmt) > 0 && parseFloat(fundTradeAmt) <= maxTopUpGross && parseFloat(fundTradeAmt) > personalBalance && (
                    <p className="mt-2 text-xs text-red-500 font-semibold flex items-center gap-1">
              ...
              <DialogFooter>
                ...
                <Button onClick={() => fundTradeMutation.mutate()}
                  disabled={fundTradeMutation.isPending || !fundTradeAmt || parseFloat(fundTradeAmt) < (selectedBroker?.minDeposit ?? TRADE_MARKET.MIN_DEPOSIT) || parseFloat(fundTradeAmt) > maxTopUpGross || parseFloat(fundTradeAmt) > personalBalance}
        ...
        <DialogContent className="sm:max-w-md max-h-[90dvh] overflow-y-auto">
          <DialogHeader>
            ...
            <DialogDescription>
              ...
              <span className="ml-2 text-emerald-600 dark:text-emerald-400 font-medium">· Withdrawable profit: <strong>${withdrawableAmt.toFixed(2)}</strong></span>
            </DialogDescription>
          </DialogHeader>
            ...
            {withdrawType === "transfer_wallet" ? (
              <>
                ...
                <div className="bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-xl p-3 text-xs text-green-800 dark:text-green-200 font-medium">
                   No fees on wallet-to-wallet transfers. Only your <strong>realised profit</strong> is available; trading capital remains locked.
                </div>
                <div className="space-y-2">
                  <Label>Amount (USD)</Label>
