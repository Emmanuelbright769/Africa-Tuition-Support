...
type View = "home" | "fund" | "send" | "request" | "pay-bill" | "service" | "send-amount" | "tsia-amount" | "tsia-otp" | "bill-otp" | "receipt" | "history" | "rates" | "crypto-withdraw" | "tx-done" | "p2p";
...
export default function FinancialHub() {
  ...
  // ── P2P Exchange state ─────────────────────────────────────────────────────
  const [p2pTab, setP2pTab]                       = useState<"market" | "offers" | "trades">("market");
  const [p2pView, setP2pView]                     = useState<"list" | "buy" | "order-view">("list");
  const [p2pCreateView, setP2pCreateView]         = useState<"list" | "create">("list");
  const [p2pOfferSelected, setP2pOfferSelected]   = useState<any>(null);
  const [p2pOrderSelected, setP2pOrderSelected]   = useState<any>(null);
  const [p2pBuyAmount, setP2pBuyAmount]           = useState("");
  const [p2pSellAmount, setP2pSellAmount]         = useState("");
  const [p2pSellRate, setP2pSellRate]             = useState("");
  const [p2pSellCurrency, setP2pSellCurrency]     = useState("NGN");
  const [p2pSellMinOrder, setP2pSellMinOrder]     = useState("5");
  const [p2pSellPaymentMethod, setP2pSellPaymentMethod] = useState("Bank Transfer");
  const [p2pSellPaymentDetails, setP2pSellPaymentDetails] = useState("");
  const [p2pCurrencyFilter, setP2pCurrencyFilter] = useState("all");
  ...
  // ── P2P queries ───────────────────────────────────────────────────────────
  const { data: p2pMarketOffers = [], refetch: refetchP2pMarket } = useQuery<any[]>({
    queryKey: ["/api/p2p/offers"],
    enabled: view === "p2p",
    staleTime: 20_000,
  });
  const { data: myP2pOffers = [], refetch: refetchMyP2pOffers } = useQuery<any[]>({
    queryKey: ["/api/p2p/offers/my"],
    enabled: view === "p2p",
    staleTime: 20_000,
  });
  const { data: myP2pOrders = [], refetch: refetchMyP2pOrders } = useQuery<any[]>({
  ...
  // ── P2P mutations ─────────────────────────────────────────────────────────
  const p2pCreateOfferMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      setP2pCreateView("list");
      setP2pSellAmount(""); setP2pSellRate(""); setP2pSellPaymentDetails("");
  ...
  const p2pPlaceOrderMutation = useMutation({
    mutationFn: async (data: any) => {
    ...
    onSuccess: (data: any) => {
      refetchMyP2pOrders();
      setP2pTab("trades");
      setP2pOrderSelected(data.order);
      setP2pView("order-view");
  ...
  const p2pMarkPaidMutation = useMutation({
  ...
  const p2pCompleteOrderMutation = useMutation({
    mutationFn: async (orderId: number) => {
  ...
  const p2pCancelOrderMutation = useMutation({
    mutationFn: async (orderId: number) => {
    ...
    onSuccess: () => {
      refetchMyP2pOrders(); refetchMyP2pOffers();
      setP2pView("list"); setP2pOrderSelected(null);
      toast({ title: "Order cancelled" });
    },
    onError: (e: any) => toast({ title: "Error", description: e.message, variant: "destructive" }),
  });

  const p2pCancelOfferMutation = useMutation({
  ...
  const p2pPauseOfferMutation = useMutation({
    mutationFn: async (offerId: number) => {
      const res = await apiRequest("PATCH", `/api/p2p/offers/${offerId}/pause`, {});
      if (!res.ok) { const e = await res.json(); throw new Error(e.message); }
      return res.json();
    },
    onSuccess: () => { refetchMyP2pOffers(); refetchP2pMarket(); },
    onError: (e: any) => toast({ title: "Error", description: e.message, variant: "destructive" }),
  });
  ...
  if (view === "home") return (
    <div className="space-y-5 pb-[76px]">
      ...
      {/* ── P2P Exchange entry ── */}
      <button
        onClick={() => { setP2pTab("market"); setP2pView("list"); setP2pCreateView("list"); setView("p2p"); }}
        className="w-full rounded-2xl bg-gradient-to-r from-tsia-green/8 to-emerald-50 dark:from-tsia-green/15 dark:to-emerald-900/20 border border-tsia-green/20 p-4 flex items-center gap-3 hover:border-tsia-green/40 active:scale-[0.99] transition-all text-left"
        data-testid="btn-p2p-market">
        <div className="w-11 h-11 rounded-2xl bg-tsia-green/15 flex items-center justify-center shrink-0">
          <Handshake className="w-6 h-6 text-tsia-green" />
        </div>
        <div className="flex-1 min-w-0">
          <p className="font-black text-sm">P2P Exchange</p>
          <p className="text-[11px] text-muted-foreground mt-0.5">Buy &amp; sell USD at custom rates · Peer-to-peer</p>
        </div>
        <div className="flex flex-col items-end gap-1 shrink-0">
          <span className="text-[10px] font-black bg-tsia-green/15 text-tsia-green px-2 py-0.5 rounded-full">Live</span>
  ...
  if (view === "p2p") {
    ...
    const getCurrInfo = (code: string) => P2P_CURRENCIES.find(c => c.code === code) ?? { code, symbol: code, name: code, flag: "🌍" };

    const statusBadge = (status: string) => {
      if (status === "pending")   return <span className="px-2 py-0.5 rounded-full bg-amber-100 dark:bg-amber-900/30 text-amber-700 dark:text-amber-400 text-[10px] font-bold uppercase tracking-wide">Pending</span>;
      if (status === "paid")      return <span className="px-2 py-0.5 rounded-full bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-400 text-[10px] font-bold uppercase tracking-wide">Paid</span>;
      if (status === "completed") return <span className="px-2 py-0.5 rounded-full bg-tsia-green/15 text-tsia-green text-[10px] font-bold uppercase tracking-wide">Completed</span>;
      return <span className="px-2 py-0.5 rounded-full bg-muted text-muted-foreground text-[10px] font-bold uppercase tracking-wide">Cancelled</span>;
    };

    // ── Order detail sub-view ──────────────────────────────────────────────
    if (p2pView === "order-view" && p2pOrderSelected) {
      const o = p2pOrderSelected;
      ...
      return (
        <AnimatePresence mode="wait">
          <motion.div key="p2p-order" initial={{ opacity:0, x:40 }} animate={{ opacity:1, x:0 }} exit={{ opacity:0, x:-40 }} className="space-y-4 pb-8">
            <BackHeader title="Order Details" onBack={() => { setP2pView("list"); setP2pOrderSelected(null); }} />

            {/* Status card */}
            <div className={`rounded-2xl p-4 border ${o.status === "completed" ? "bg-tsia-green/10 border-tsia-green/30" : o.status === "paid" ? "bg-blue-50 dark:bg-blue-900/20 border-blue-200 dark:border-blue-700/30" : o.status === "cancelled" ? "bg-muted border-border" : "bg-amber-50 dark:bg-amber-900/20 border-amber-200 dark:border-amber-700/30"}`}>
              <div className="flex items-center justify-between mb-2">
                <p className="font-black text-sm">Order #{o.id}</p>
                {statusBadge(o.status)}
              </div>
              <p className="text-2xl font-black">${usdAmt.toFixed(2)} USD</p>
              <p className="text-xs text-muted-foreground mt-0.5">≈ {ci.symbol}{localAmt.toLocaleString(undefined, { maximumFractionDigits: 0 })} {o.localCurrency} · Rate: {ci.symbol}{parseFloat(o.ratePerUsd).toLocaleString()}/USD</p>
            </div>

            {/* Parties */}
            <div className="rounded-2xl bg-card border border-border p-4">
              <p className="font-bold text-[10px] uppercase tracking-wider text-muted-foreground mb-3">Trade Parties</p>
              <div className="flex items-center justify-between gap-2">
                <div className="flex items-center gap-2.5 flex-1 min-w-0">
                  <div className="w-9 h-9 rounded-full bg-tsia-green/10 flex items-center justify-center shrink-0">
                    <span className="font-black text-tsia-green text-sm">{o.sellerName[0]}</span>
                  </div>
                  <div className="min-w-0">
                    <p className="font-semibold text-sm truncate">{o.sellerName.split(" ")[0]} {o.sellerName.split(" ")[1]?.[0]}. {isSeller ? <span className="text-tsia-green text-[10px]">(You)</span> : ""}</p>
                ...
                <div className="flex items-center gap-2.5 flex-1 min-w-0 justify-end">
                  <div className="min-w-0 text-right">
                    <p className="font-semibold text-sm truncate">{o.buyerName.split(" ")[0]} {o.buyerName.split(" ")[1]?.[0]}. {isBuyer ? <span className="text-blue-600 text-[10px]">(You)</span> : ""}</p>
                  ...
                  <div className="w-9 h-9 rounded-full bg-blue-100 dark:bg-blue-900/30 flex items-center justify-center shrink-0">
                    <span className="font-black text-blue-600 dark:text-blue-400 text-sm">{o.buyerName[0]}</span>
                  </div>
                </div>
              </div>
            </div>

            {/* Payment instructions for pending buyer */}
            {isBuyer && o.status === "pending" && (
              <div className="rounded-2xl bg-card border border-amber-300/50 dark:border-amber-700/30 p-4 space-y-3">
                <div className="flex items-center gap-2">
                  ...
                  <p className="font-bold text-sm">Payment Instructions</p>
                </div>
                <p className="text-xs text-muted-foreground leading-relaxed">
                  Send exactly <span className="font-bold text-foreground">{ci.symbol}{localAmt.toLocaleString(undefined, { maximumFractionDigits: 0 })} {o.localCurrency}</span> via <span className="font-bold text-foreground">{o.paymentMethod}</span>:
                </p>
                <div className="bg-muted/60 rounded-xl p-3">
                  <p className="text-xs font-mono text-foreground whitespace-pre-line leading-relaxed">{o.paymentDetails}</p>
                </div>
                <div className="flex items-start gap-2 bg-amber-50 dark:bg-amber-950/30 rounded-xl p-3">
                  <AlertCircle className="w-3.5 h-3.5 text-amber-600 dark:text-amber-400 shrink-0 mt-0.5" />
                  <p className="text-[10px] text-amber-700 dark:text-amber-400 leading-relaxed">Only click "I've Paid" after completing the payment. False confirmations may result in account suspension.</p>
                </div>
              </div>
            )}

            {/* Waiting state for paid-buyer */}
            {isBuyer && o.status === "paid" && (
              <div className="rounded-2xl bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-700/30 p-4 flex gap-3">
                <Clock className="w-5 h-5 text-blue-600 dark:text-blue-400 shrink-0 mt-0.5" />
                <div>
                  <p className="font-bold text-sm text-blue-700 dark:text-blue-400">Awaiting Seller Confirmation</p>
                  <p className="text-xs text-blue-600/80 dark:text-blue-400/70 mt-0.5">The seller will verify your payment and release the USD.</p>
                </div>
              </div>
            )}
            ...
            {isSeller && o.status === "paid" && (
              <div className="rounded-2xl bg-tsia-green/10 border border-tsia-green/30 p-4 space-y-2">
                <div className="flex items-center gap-2">
                  ...
                  <p className="font-bold text-sm">Buyer Has Paid!</p>
                </div>
                <p className="text-xs text-muted-foreground leading-relaxed">
                  {o.buyerName.split(" ")[0]} has confirmed sending <span className="font-bold text-foreground">{ci.symbol}{localAmt.toLocaleString(undefined, { maximumFractionDigits: 0 })} {o.localCurrency}</span> via {o.paymentMethod}. Once you verify receipt, release the USD.
                </p>
              </div>
            )}
            ...
            {o.status === "completed" && (
              <div className="rounded-2xl bg-tsia-green/10 border border-tsia-green/30 p-4 flex items-center gap-3">
                ...
                <div>
                  <p className="font-bold text-sm">Trade Completed!</p>
                  <p className="text-xs text-muted-foreground mt-0.5">${usdAmt.toFixed(2)} transferred to {isSeller ? "the buyer's" : "your"} wallet.</p>
                </div>
              </div>
            )}
            ...
            <div className="space-y-2">
              {isBuyer && o.status === "pending" && (
                <Button className="w-full bg-gradient-to-r from-tsia-green to-emerald-500 text-white font-bold rounded-xl h-12"
                  onClick={() => p2pMarkPaidMutation.mutate(o.id)}
                  disabled={p2pMarkPaidMutation.isPending}>
                  {p2pMarkPaidMutation.isPending ? <><Loader2 className="w-4 h-4 mr-2 animate-spin" />Confirming…</> : <><Check className="w-4 h-4 mr-2" />I&apos;ve Paid</>}
                </Button>
              )}
              {isSeller && o.status === "paid" && (
                <Button className="w-full bg-gradient-to-r from-tsia-green to-emerald-500 text-white font-bold rounded-xl h-12"
                  onClick={() => p2pCompleteOrderMutation.mutate(o.id)}
                  disabled={p2pCompleteOrderMutation.isPending}>
                  {p2pCompleteOrderMutation.isPending ? <><Loader2 className="w-4 h-4 mr-2 animate-spin" />Releasing…</> : <><Coins className="w-4 h-4 mr-2" />Release USD to Buyer</>}
                </Button>
              )}
              {(o.status === "pending" || o.status === "paid") && (
                <Button variant="outline" className="w-full rounded-xl border-red-300 dark:border-red-800/50 text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-950/20"
                  onClick={() => p2pCancelOrderMutation.mutate(o.id)}
                  disabled={p2pCancelOrderMutation.isPending}>
                  {p2pCancelOrderMutation.isPending ? <><Loader2 className="w-4 h-4 mr-2 animate-spin" />Cancelling…</> : "Cancel Order"}
                </Button>
              )}
            </div>
          </motion.div>
    ...
    // ── Buy sub-view ───────────────────────────────────────────────────────
    if (p2pTab === "market" && p2pView === "buy" && p2pOfferSelected) {
      const offer   = p2pOfferSelected;
      const ci      = getCurrInfo(offer.localCurrency);
      const buyAmt  = parseFloat(p2pBuyAmount) || 0;
      ...
      return (
        <AnimatePresence mode="wait">
          <motion.div key="p2p-buy" initial={{ opacity:0, x:40 }} animate={{ opacity:1, x:0 }} exit={{ opacity:0, x:-40 }} className="space-y-4 pb-8">
            <BackHeader title="Place Order" onBack={() => { setP2pView("list"); setP2pOfferSelected(null); setP2pBuyAmount(""); }} />

            {/* Seller card */}
            <div className="rounded-2xl bg-card border border-border p-4 flex items-center gap-3">
              <div className="w-11 h-11 rounded-full bg-tsia-green/10 flex items-center justify-center shrink-0">
                <span className="font-black text-tsia-green text-base">{offer.sellerName[0]}</span>
              </div>
              <div className="flex-1 min-w-0">
                <p className="font-bold text-sm">{offer.sellerName.split(" ")[0]} {offer.sellerName.split(" ")[1]?.[0]}.</p>
                <div className="flex items-center gap-1.5 mt-0.5 flex-wrap">
                  ...
                  {offer.completedTrades > 0 && <span className="text-[10px] text-muted-foreground">· {offer.completedTrades} trades</span>}
                  <span className="text-[10px] bg-muted text-muted-foreground px-1.5 py-0.5 rounded-full">{offer.paymentMethod}</span>
                </div>
              </div>
              <div className="text-right shrink-0">
                <p className="font-black text-tsia-green">{ci.symbol}{parseFloat(offer.ratePerUsd).toLocaleString()}</p>
                <p className="text-[10px] text-muted-foreground">per USD</p>
              </div>
            </div>

            {/* Amount input */}
            <div className="rounded-2xl bg-card border border-border p-4 space-y-3">
              <Label className="text-xs font-semibold">Amount to Buy (USD)</Label>
              <div className="relative">
                <span className="absolute left-3 top-1/2 -translate-y-1/2 font-black text-muted-foreground">$</span>
                <Input type="number" min={minOrd} max={maxOrd} step="0.01"
                  value={p2pBuyAmount} onChange={e => setP2pBuyAmount(e.target.value)}
                  placeholder={`${minOrd}–${maxOrd.toFixed(2)}`}
                  className="pl-7 h-12 text-lg font-bold" />
              </div>
              <div className="flex justify-between text-xs text-muted-foreground">
                <span>Min: <span className="font-bold text-foreground">${minOrd}</span></span>
                <span>Max: <span className="font-bold text-foreground">${maxOrd.toFixed(2)}</span></span>
              </div>
              {buyAmt > 0 && (
                <div className="rounded-xl bg-tsia-green/8 dark:bg-tsia-green/15 p-3 flex items-center justify-between">
                  <span className="text-xs text-muted-foreground font-semibold">You&apos;ll pay</span>
                  <span className="font-black text-lg">{ci.symbol}{(buyAmt * rate).toLocaleString(undefined, { maximumFractionDigits: 0 })} <span className="text-xs text-muted-foreground font-semibold">{offer.localCurrency}</span></span>
                </div>
              )}
            </div>
            ...
            {/* Payment method note */}
            <div className="rounded-2xl bg-card border border-border p-4">
              <p className="font-bold text-xs uppercase tracking-wider text-muted-foreground mb-1.5">Payment Method</p>
              <p className="font-semibold text-sm">{offer.paymentMethod}</p>
              <p className="text-[11px] text-muted-foreground mt-1">Payment details will be shown after order placement.</p>
            </div>

            <Button className="w-full bg-gradient-to-r from-tsia-green to-emerald-500 text-white font-bold rounded-xl h-12"
              disabled={!isValid || p2pPlaceOrderMutation.isPending}
              onClick={() => p2pPlaceOrderMutation.mutate({ offerId: offer.id, amountUsd: buyAmt })}>
              {p2pPlaceOrderMutation.isPending
                ? <><Loader2 className="w-4 h-4 mr-2 animate-spin" />Placing Order…</>
                : <><Handshake className="w-4 h-4 mr-2" />Place Order · ${buyAmt > 0 ? buyAmt.toFixed(2) : "—"}</>}
            </Button>
            {!isValid && buyAmt > 0 && (
              <p className="text-center text-xs text-red-500">Amount must be between ${minOrd} and ${maxOrd.toFixed(2)}</p>
            )}
          </motion.div>
        </AnimatePresence>
      );
    ...
    if (p2pTab === "offers" && p2pCreateView === "create") {
      const sellAmt  = parseFloat(p2pSellAmount) || 0;
      const sellRate = parseFloat(p2pSellRate) || 0;
      const minOrd   = parseFloat(p2pSellMinOrder) || 5;
      const ci       = getCurrInfo(p2pSellCurrency);
      const ledger   = Math.min(balance, 2);
      const avail    = Math.max(0, balance - ledger);
      const isValid  = sellAmt >= 5 && sellAmt <= avail && sellRate > 0 && p2pSellPaymentMethod.trim() !== "" && p2pSellPaymentDetails.trim() !== "";
      return (
        <AnimatePresence mode="wait">
          <motion.div key="p2p-create" initial={{ opacity:0, x:40 }} animate={{ opacity:1, x:0 }} exit={{ opacity:0, x:-40 }} className="space-y-4 pb-8">
            <BackHeader title="Create Sell Offer" onBack={() => setP2pCreateView("list")} />

            {/* Wallet info */}
            <div className="rounded-2xl bg-gradient-to-br from-tsia-green/10 to-emerald-50 dark:from-tsia-green/15 dark:to-emerald-900/20 border border-tsia-green/20 p-4">
              <p className="text-[10px] text-muted-foreground font-semibold uppercase tracking-wide mb-0.5">Available to list</p>
              <p className="text-2xl font-black">${avail.toFixed(2)}</p>
              <p className="text-[10px] text-muted-foreground mt-0.5">(After $2 ledger reserve)</p>
            </div>

            {/* Amount */}
            <div className="rounded-2xl bg-card border border-border p-4 space-y-2">
              <Label className="text-xs font-semibold">Amount to Sell (USD)</Label>
              <div className="relative">
                <span className="absolute left-3 top-1/2 -translate-y-1/2 font-black text-muted-foreground">$</span>
                <Input type="number" min="5" max={avail} step="0.01"
                  value={p2pSellAmount} onChange={e => setP2pSellAmount(e.target.value)}
                  placeholder="e.g. 50" className="pl-7 h-11 font-bold" />
              </div>
              <p className="text-[10px] text-muted-foreground">Minimum $5 · Max ${avail.toFixed(2)}</p>
            </div>

            {/* Currency + Rate */}
            <div className="rounded-2xl bg-card border border-border p-4 space-y-3">
              <div>
                <Label className="text-xs font-semibold">Local Currency</Label>
                <div className="grid grid-cols-4 gap-2 mt-2">
                  {P2P_CURRENCIES.map(c => (
                    <button key={c.code} onClick={() => setP2pSellCurrency(c.code)}
                      className={`rounded-xl border py-2 text-[11px] font-bold transition-all ${p2pSellCurrency === c.code ? "border-tsia-green bg-tsia-green/10 text-tsia-green" : "border-border bg-muted/30 text-muted-foreground"}`}>
                      {c.flag} {c.code}
                    </button>
                  ))}
                </div>
              ...
              <div>
                <Label className="text-xs font-semibold">Your Rate ({ci.symbol} per $1 USD)</Label>
                <div className="relative mt-1.5">
                  <span className="absolute left-3 top-1/2 -translate-y-1/2 text-xs font-bold text-muted-foreground">{ci.symbol}</span>
                  <Input type="number" min="1" step="1"
                    value={p2pSellRate} onChange={e => setP2pSellRate(e.target.value)}
                    placeholder="e.g. 1650" className="pl-7 h-11 font-bold" />
                </div>
                {sellAmt > 0 && sellRate > 0 && (
                  <p className="text-[11px] text-muted-foreground mt-1.5">Buyer pays <span className="font-bold text-foreground">{ci.symbol}{(sellAmt * sellRate).toLocaleString(undefined, { maximumFractionDigits: 0 })}</span> for ${sellAmt.toFixed(2)} USD</p>
                )}
              </div>
            </div>
            ...
            {/* Min order */}
            <div className="rounded-2xl bg-card border border-border p-4 space-y-2">
              <Label className="text-xs font-semibold">Minimum Order Size (USD)</Label>
              <div className="relative">
                <span className="absolute left-3 top-1/2 -translate-y-1/2 font-black text-muted-foreground">$</span>
                <Input type="number" min="1" max={sellAmt || 100} step="1"
                  value={p2pSellMinOrder} onChange={e => setP2pSellMinOrder(e.target.value)}
                  placeholder="5" className="pl-7 h-11 font-bold" />
              </div>
            </div>
            ...
            <div className="rounded-2xl bg-card border border-border p-4 space-y-3">
              <div>
                ...
                <div className="grid grid-cols-3 gap-2 mt-2">
                  {["Bank Transfer", "Mobile Money", "Cash"].map(m => (
                    <button key={m} onClick={() => setP2pSellPaymentMethod(m)}
                      className={`rounded-xl border py-2 text-[10px] font-bold transition-all leading-tight ${p2pSellPaymentMethod === m ? "border-tsia-green bg-tsia-green/10 text-tsia-green" : "border-border bg-muted/30 text-muted-foreground"}`}>
                      {m}
                    </button>
                  ))}
                </div>
              ...
              <div>
                <Label className="text-xs font-semibold">Payment Details <span className="text-muted-foreground font-normal">(shown to buyer)</span></Label>
                <textarea
                  value={p2pSellPaymentDetails}
                  onChange={e => setP2pSellPaymentDetails(e.target.value)}
                  placeholder={"e.g.\nBank: GTBank\nAccount: 0123456789\nName: John Doe"}
                  className="mt-1.5 w-full h-24 text-xs font-mono bg-muted/40 border border-border rounded-xl p-3 resize-none focus:outline-none focus:ring-2 focus:ring-tsia-green/50 transition-all" />
              </div>
            </div>

            <Button className="w-full bg-gradient-to-r from-tsia-green to-emerald-500 text-white font-bold rounded-xl h-12"
              disabled={!isValid || p2pCreateOfferMutation.isPending}
              onClick={() => p2pCreateOfferMutation.mutate({
                amountUsd: sellAmt, ratePerUsd: sellRate, localCurrency: p2pSellCurrency,
                minOrderUsd: minOrd, maxOrderUsd: sellAmt, paymentMethod: p2pSellPaymentMethod,
                paymentDetails: p2pSellPaymentDetails,
              })}>
              {p2pCreateOfferMutation.isPending
                ? <><Loader2 className="w-4 h-4 mr-2 animate-spin" />Creating…</>
                : <><Coins className="w-4 h-4 mr-2" />List ${sellAmt > 0 ? sellAmt.toFixed(2) : "—"} for Sale</>}
            </Button>
            {sellAmt > 0 && sellAmt > avail && (
              <p className="text-center text-xs text-red-500">Amount exceeds available balance (${avail.toFixed(2)})</p>
            )}
          </motion.div>
        </AnimatePresence>
      );
    ...
    return (
      <AnimatePresence mode="wait">
        <motion.div key="p2p-main" initial={{ opacity:0, x:40 }} animate={{ opacity:1, x:0 }} exit={{ opacity:0, x:-40 }} className="space-y-4 pb-8">

          {/* Header */}
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              <button onClick={() => { setView("home"); setP2pView("list"); setP2pCreateView("list"); }}
                className="w-8 h-8 rounded-full bg-muted flex items-center justify-center hover:bg-muted/80 transition-colors">
                <ArrowLeft className="w-4 h-4" />
              </button>
              <div>
                <h2 className="font-black text-base leading-tight">P2P Exchange</h2>
                <p className="text-[10px] text-muted-foreground">Buy &amp; sell USD at custom rates</p>
              </div>
            </div>
            <div className="w-9 h-9 rounded-full bg-tsia-green/10 flex items-center justify-center">
              <Handshake className="w-5 h-5 text-tsia-green" />
            </div>
          </div>

          {/* Tab bar */}
          <div className="flex bg-muted/50 rounded-2xl p-1">
            {(["market", "offers", "trades"] as const).map(tab => (
              <button key={tab} onClick={() => {
                setP2pTab(tab);
                if (tab === "market") { refetchP2pMarket(); setP2pView("list"); }
                if (tab === "offers") refetchMyP2pOffers();
                if (tab === "trades") refetchMyP2pOrders();
              }}
                className={`flex-1 py-2.5 rounded-xl text-xs font-bold transition-all ${p2pTab === tab ? "bg-card shadow text-foreground" : "text-muted-foreground hover:text-foreground"}`}>
                {tab === "market" ? "Market" : tab === "offers" ? "My Offers" : "My Trades"}
              </button>
            ))}
          </div>
          ...
          {p2pTab === "market" && (
            <div className="space-y-3">
              ...
              <div className="flex gap-2 overflow-x-auto pb-1 -mx-1 px-1 scrollbar-none">
                <button onClick={() => setP2pCurrencyFilter("all")}
                  className={`px-3 py-1.5 rounded-full text-[11px] font-bold shrink-0 transition-all ${p2pCurrencyFilter === "all" ? "bg-tsia-green text-white" : "bg-muted text-muted-foreground"}`}>
                  All
                </button>
                {P2P_CURRENCIES.map(c => (
                  <button key={c.code} onClick={() => setP2pCurrencyFilter(c.code)}
                    className={`px-3 py-1.5 rounded-full text-[11px] font-bold shrink-0 flex items-center gap-1 transition-all ${p2pCurrencyFilter === c.code ? "bg-tsia-green text-white" : "bg-muted text-muted-foreground"}`}>
                    {c.flag} {c.code}
                  </button>
                ))}
              </div>
              ...
              {/* Offer list */}
              {(() => {
                const filtered = p2pMarketOffers
                  .filter(o => p2pCurrencyFilter === "all" || o.localCurrency === p2pCurrencyFilter)
                ...
                if (filtered.length === 0) return (
                  <div className="text-center py-14">
                    ...
                    <p className="text-xs text-muted-foreground mt-1 mb-4">Be the first — sell your USD and set your own rate!</p>
                    <Button onClick={() => { setP2pTab("offers"); setP2pCreateView("create"); }}
                      className="bg-tsia-green text-white rounded-xl font-bold text-xs px-5 h-9">
                      Create an Offer
                    </Button>
                  </div>
                );
                return (
                  <div className="space-y-3">
                    {filtered.map(offer => {
                      const ci = getCurrInfo(offer.localCurrency);
                      return (
                        <div key={offer.id} className="rounded-2xl bg-card border border-border p-4 space-y-3 hover:border-tsia-green/30 transition-all">
                          <div className="flex items-center justify-between">
                            <div className="flex items-center gap-2.5">
                          ...
                          <div className="border-t border-border pt-2.5 flex items-center justify-between">
                            <div className="flex items-center gap-4 text-xs">
                              <div>
                                ...
                                <p className="font-bold">${parseFloat(offer.availableUsd).toFixed(2)}</p>
                              </div>
                              <div>
                                <p className="text-[10px] text-muted-foreground uppercase tracking-wide">Limit</p>
                                <p className="font-bold">${parseFloat(offer.minOrderUsd).toFixed(0)}–${parseFloat(offer.maxOrderUsd).toFixed(0)}</p>
                              </div>
                            </div>
                            <button onClick={() => { setP2pOfferSelected(offer); setP2pView("buy"); setP2pBuyAmount(""); }}
                              className="px-4 py-2 rounded-xl bg-tsia-green text-white text-xs font-black hover:bg-tsia-green/90 active:scale-95 transition-all">
                              Buy USD
                            </button>
                          </div>
                          <div className="flex items-center gap-1.5 -mt-1">
                            <span className="text-[10px] bg-muted text-muted-foreground px-2 py-0.5 rounded-full">{offer.paymentMethod}</span>
                            <span className="text-[10px] bg-muted text-muted-foreground px-2 py-0.5 rounded-full">{ci.flag} {offer.localCurrency}</span>
                          </div>
                        </div>
                      );
                    })}
          ...
          {p2pTab === "offers" && (
            <div className="space-y-3">
              <Button onClick={() => { setP2pCreateView("create"); setP2pSellAmount(""); setP2pSellRate(""); setP2pSellPaymentDetails(""); setP2pSellCurrency("NGN"); setP2pSellMinOrder("5"); setP2pSellPaymentMethod("Bank Transfer"); }}
                className="w-full bg-gradient-to-r from-tsia-green to-emerald-500 text-white font-bold rounded-xl">
                <Plus className="w-4 h-4 mr-2" /> Create New Offer
              </Button>
              ...
              {myP2pOffers.length === 0 ? (
                <div className="text-center py-14">
                  ...
                  <p className="font-bold text-sm">No offers yet</p>
                  <p className="text-xs text-muted-foreground mt-1">Create an offer to sell your USD at your own rate</p>
                </div>
              ) : (
                <div className="space-y-3">
                  {myP2pOffers.map(offer => {
                    ...
                    return (
                      <div key={offer.id} className={`rounded-2xl border p-4 space-y-3 transition-all ${isCancelled ? "bg-muted/30 border-border opacity-60" : "bg-card border-border"}`}>
                        <div className="flex items-start justify-between">
                          <div>
                            <div className="flex items-center gap-2 flex-wrap">
                              <p className="font-black text-base">${parseFloat(offer.amountUsd).toFixed(2)} USD</p>
                              {offer.status === "active"    && <span className="text-[10px] px-2 py-0.5 rounded-full bg-tsia-green/15 text-tsia-green font-bold">Active</span>}
                              {offer.status === "paused"    && <span className="text-[10px] px-2 py-0.5 rounded-full bg-amber-100 dark:bg-amber-900/30 text-amber-700 dark:text-amber-400 font-bold">Paused</span>}
                              {offer.status === "cancelled" && <span className="text-[10px] px-2 py-0.5 rounded-full bg-muted text-muted-foreground font-bold">Cancelled</span>}
                              {offer.status === "completed" && <span className="text-[10px] px-2 py-0.5 rounded-full bg-tsia-green/15 text-tsia-green font-bold">Sold Out</span>}
                            </div>
                            <p className="text-xs text-muted-foreground mt-1">Rate: {ci.symbol}{parseFloat(offer.ratePerUsd).toLocaleString()} · Available: ${parseFloat(offer.availableUsd).toFixed(2)}</p>
                          </div>
                          <div className="text-right text-xs shrink-0 ml-2">
                            <p className="font-bold text-muted-foreground">{offer.completedTrades} trade{offer.completedTrades !== 1 ? "s" : ""}</p>
                            <p className="text-muted-foreground mt-0.5">{ci.flag} {offer.localCurrency}</p>
                          </div>
                        </div>
                        {!isCancelled && offer.status !== "completed" && (
                          <div className="flex gap-2 border-t border-border pt-3">
                            <button onClick={() => p2pPauseOfferMutation.mutate(offer.id)}
                              disabled={p2pPauseOfferMutation.isPending}
                              className="flex-1 py-2 rounded-xl border border-border text-xs font-bold text-muted-foreground hover:bg-muted/50 transition-all">
                              {offer.status === "paused" ? "Resume" : "Pause"}
                            </button>
                            <button onClick={() => p2pCancelOfferMutation.mutate(offer.id)}
                              disabled={p2pCancelOfferMutation.isPending}
                              className="flex-1 py-2 rounded-xl border border-red-200 dark:border-red-800/50 text-xs font-bold text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-950/20 transition-all">
                              Cancel &amp; Return Funds
                            </button>
                          </div>
                        )}
          ...
          {p2pTab === "trades" && (
            <div className="space-y-3">
              {myP2pOrders.length === 0 ? (
                <div className="text-center py-14">
                  <div className="w-16 h-16 rounded-full bg-muted/60 flex items-center justify-center mx-auto mb-3">
                    <ArrowLeftRight className="w-8 h-8 text-muted-foreground" />
                  </div>
                  <p className="font-bold text-sm">No trades yet</p>
                  <p className="text-xs text-muted-foreground mt-1 mb-4">Browse the market to buy USD at great rates</p>
                  <Button onClick={() => setP2pTab("market")} className="bg-tsia-green text-white rounded-xl font-bold text-xs px-5 h-9">
                    Browse Market
                  </Button>
                </div>
              ) : (
                <div className="space-y-3">
                  {myP2pOrders.map(order => {
                    ...
                    return (
                      <button key={order.id}
                        onClick={() => { setP2pOrderSelected(order); setP2pView("order-view"); }}
                        className={`w-full rounded-2xl border p-4 text-left transition-all active:scale-[0.99] ${needsAction ? "border-tsia-green/40 bg-tsia-green/5 dark:bg-tsia-green/10" : "border-border bg-card hover:border-tsia-green/30"}`}>
                        <div className="flex items-center justify-between mb-2">
                          <div>
                            <div className="flex items-center gap-1.5 mb-0.5">
                              {needsAction && <div className="w-1.5 h-1.5 rounded-full bg-tsia-green animate-pulse" />}
                              <p className="font-black text-sm">{isBuyer ? "Buy" : "Sell"} ${usdAmt.toFixed(2)} USD</p>
                            </div>
                          ...
                          <div className="flex flex-col items-end gap-1">
                            ...
                            {needsAction && (
                              <span className="text-[9px] font-black text-tsia-green uppercase tracking-wide">Action needed</span>
                            )}
                          </div>
                        </div>
