...
- **Admin Trade Market Controls (T003)**: `GET /api/admin/trade-settings` + `PUT /api/admin/trade-settings` routes store exchange withdraw fee, bank withdraw fee, reserve rate, affiliate share rate, min deposit, min withdraw, and co-affiliate pool rate override in `platform_settings` table; new "Trade Market Rate Controls" card in admin Settings tab with live editing + Save button; co-affiliate pool rate override field (blank = auto-tiered)
- **Terms of Service Update (T007)**: Effective date updated to 1 May 2026; added clauses 7.8 (Virtual Mastercard), 7.9 (Netflix access), 7.10 (deposit allocation clarification); `POST /api/admin/email-tos-update` route sends Brevo emails to all registered users; new "Terms of Service — Notify All Users" card in admin Settings tab
- **Fee Structure Verified (T006)**: Service charge routes (airtime, data, bills) confirmed to use `fee: "0.00"`; wallet deposits maintain 75/20/5 split; no additional changes needed
- **WalletPage.tsx syntax fix**: Corrected `</>));` → `</>);` at deposits IIFE return — pre-existing syntax error from session 9 pagination work
